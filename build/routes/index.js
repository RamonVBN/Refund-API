...
var RefundsController = class {
  async create(request, response) {
    const bodySchema = import_zod3.z.object({
      name: import_zod3.z.string().trim().min(3),
      category: import_zod3.z.enum(["food", "others", "services", "transport", "accommodation"]),
      amount: import_zod3.z.number().positive({ message: "O valor precisa ser positivo." }),
      filename: import_zod3.z.string().min(20)
    });
  ...
  async index(request, response) {
    const querySchema = import_zod3.z.object({
      name: import_zod3.z.string().optional().default(""),
      page: import_zod3.z.coerce.number().optional().default(1),
      perPage: import_zod3.z.coerce.number().optional().default(10)
    });
...
var UploadsController = class {
  async create(request, response) {
    ...
    try {
      const fileSchema = import_zod4.z.object({
        filename: import_zod4.z.string().min(1, "Arquivo \xE9 obrigat\xF3rio"),
        mimetype: import_zod4.z.string().refine((type) => upload_default.ACCEPTED_IMAGE_TYPES.includes(type), "Formato de arquivo inv\xE1lido. Formatos permitidos: " + upload_default.ACCEPTED_IMAGE_TYPES),
        size: import_zod4.z.number().positive().refine((size) => size <= upload_default.MAX_FILE_SIZE, "Arquivo excede o tamanho m\xE1ximo de " + upload_default.MAX_SIZE)
      }).passthrough();
